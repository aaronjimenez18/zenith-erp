      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Ventas Totales</p>
            <div className="p-2 bg-green-50 rounded-lg text-green-600">
              <TrendingUp size={20} />
            </div>
          </div>
          <p className="text-3xl font-bold text-slate-900">
            ${(totalSales._sum.total || 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
          </p>
          <p className="text-xs text-green-600 mt-2 font-medium">Acumulado histórico</p>
        </div>

        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Productos</p>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className={`p-2 rounded-lg ${lowStockProducts.length > 0 ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600"}`}>
              <AlertCircle size={20} />
            </div>
          </div>
          <p className={`text-3xl font-bold ${lowStockProducts.length > 0 ? "text-red-500" : "text-emerald-600"}`}>
            {lowStockProducts.length} <span className="text-xl font-medium">Críticos</span>
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900">Inventario en Riesgo</h2>
          </div>
          <div className="p-6">
            <div className="space-y-1">
              {lowStockProducts.length === 0 ? (
                <p className="text-slate-500 text-sm py-4 text-center">Todo el stock está al día.</p>
              ) : (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6">
            <div className="space-y-1">
              {recentSales.map((s) => (
                <div key={s.id} className="flex justify-between items-center py-4 border-b border-slate-50 last:border-0">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400">
                      <History size={16} />
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-slate-700">Venta Registrada</p>
